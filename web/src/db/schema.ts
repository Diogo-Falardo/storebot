import { mysqlTable, mysqlSchema, AnyMySqlColumn, index, foreignKey, primaryKey, char, datetime, varchar, decimal, text, mysqlEnum, unique, bigint } from "drizzle-orm/mysql-core"
import { sql } from "drizzle-orm"

export const linkTokens = mysqlTable("link_tokens", {
	tokenHash: char("token_hash", { length: 36 }).default(sql`(uuid())`).notNull(),
...
export const shops = mysqlTable("shops", {
	id: char({ length: 36 }).default(sql`(uuid())`).notNull(),
	userId: char("user_id", { length: 36 }).notNull().references(() => users.id, { onDelete: "cascade" } ),
	shopType: mysqlEnum("shop_type", ['public','private','telegram_only']).default('public').notNull(),
