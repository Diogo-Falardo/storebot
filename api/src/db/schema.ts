import { mysqlTable, mysqlSchema, AnyMySqlColumn, foreignKey, primaryKey, char, varchar, index, bigint, datetime, decimal, text, tinyint, mysqlEnum, unique } from "drizzle-orm/mysql-core"
...
export const orders = mysqlTable("orders", {
	id: char({ length: 36 }).notNull(),
	shopId: char("shop_id", { length: 36 }).notNull().references(() => shops.id, { onDelete: "cascade" } ),
	...
	orderDeliveryInstruction: varchar("order_delivery_instruction", { length: 1000 }).notNull(),
	orderCustomMessage: varchar("order_custom_message", { length: 2500 }),
	orderShippingMethod: char("order_shipping_method", { length: 36 }).notNull().references(() => shippingMethods.id, { onDelete: "cascade" } ),
	orderPaymentMethod: char("order_payment_method", { length: 36 }).notNull().references(() => paymentMethods.id, { onDelete: "cascade" } ),
...
export const productsOrders = mysqlTable("products_orders", {
	id: char({ length: 36 }).notNull(),
